...
module.exports = async function handler(req, res) {
    ...
    const formatSong = (song) => ({
        ...
        coverArtistUsername: song.cover_artist_username,
    ...
    try {
        ...
        if (req.method === 'GET' && path.startsWith('user/')) {
            ...
            const [songs] = await pool.query(`
                SELECT s.*, u.name as cover_artist, u.username as cover_artist_username, a.cover_image as album_cover, a.title as album_title
                FROM songs s
        ...
        if (req.method === 'GET' && path === '') {
            const [songs] = await pool.query(`
                SELECT s.*, u.name as cover_artist, u.username as cover_artist_username, a.title as album_title, a.cover_image as album_cover
                FROM songs s
                LEFT JOIN users u ON s.user_id = u.id
                LEFT JOIN albums a ON s.album_id = a.id
        ...
        if (req.method === 'POST' && path.endsWith('/like')) {
            ...
            if (existing.length > 0) {
                ...
                try {
                    const [songs] = await pool.query('SELECT user_id, title FROM songs WHERE id = ?', [id]);
                    if (songs.length > 0 && songs[0].user_id !== user.id) {
                        await pool.query(
                            ...
                            [songs[0].user_id, user.id, 'like', id, `${user.name} menyukai lagu "${songs[0].title}"`]
                        );
                    }
                } catch (notifError) {
